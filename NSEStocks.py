...
def get_top_20_indian_stocks():
    """Get top 20 Indian stocks by market cap"""
    top_20_stocks = [
        'RELIANCE.NS', 'TCS.NS', 'HDFCBANK.NS', 'INFY.NS', 'ICICIBANK.NS',
        'HINDUNILVR.NS', 'SBIN.NS', 'BHARTIARTL.NS', 'ITC.NS', 'KOTAKBANK.NS',
        'HCLTECH.NS', 'BAJFINANCE.NS', 'AXISBANK.NS', 'ASIANPAINT.NS', 'MARUTI.NS',
        'TITAN.NS', 'LT.NS', 'NTPC.NS', 'ONGC.NS', 'SUNPHARMA.NS'
    ]
...
def calculate_relative_strength(stock_data, nifty_data):
    """Calculate Relative Strength Ratio (Stock vs Nifty 50)"""
    stock_returns = stock_data['Close'].pct_change()
    nifty_returns = nifty_data['Close'].pct_change()
    
    # Calculate relative strength (stock performance vs Nifty)
    relative_strength = (1 + stock_returns) / (1 + nifty_returns)
    relative_strength = relative_strength.cumprod()
    
    return relative_strength

def check_nifty_above_ema200():
    """Check if Nifty 50 is above its 200-EMA on daily timeframe"""
    try:
        nifty = yf.Ticker('^NSEI')
        nifty_data = nifty.history(period='300d', interval='1d')
        
        if len(nifty_data) < 200:
            return False
        
        nifty_data['EMA200'] = calculate_ema(nifty_data['Close'], 200)
        latest = nifty_data.iloc[-1]
        
        return latest['Close'] > latest['EMA200']
        
    except Exception as e:
        print(f"Error checking Nifty condition: {e}")
        return False

def check_15m_conditions(ticker_symbol, nifty_data):
    ...
    try:
        ...
        relative_strength = calculate_relative_strength(data_15m, nifty_data)
...
def analyze_stocks():
    ...
    print("Fetching top 20 Indian stocks...")
    stocks = get_top_20_indian_stocks()
    
    print("Checking Nifty 50 condition...")
    nifty_condition = check_nifty_above_ema200()
    if not nifty_condition:
        print("❌ Nifty 50 is not above its 200-EMA. Market condition not favorable.")
        return []
    
    print("✓ Nifty 50 is above its 200-EMA. Proceeding with analysis...")
    
    # Get Nifty data for relative strength calculation
    nifty = yf.Ticker('^NSEI')
    nifty_data_15m = nifty.history(period='30d', interval='15m')
    ...
    for i, ticker in enumerate(stocks, 1):
        ...
        (condition_15m, rel_strength, adx_value, rsi_value, 
         ...
         bb_percent, bb_width) = check_15m_conditions(ticker, nifty_data_15m)
...
def main():
    ...
    print("Indian Stock Screening Tool")
    print("=" * 90)
    print("Filters:")
    print("- Market: Nifty 50 > EMA200 (Daily)")
    ...
    print("- Relative Strength Ratio (vs Nifty 50)")
    print("=" * 90)
